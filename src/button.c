...
void button_update(void) {
  ...
  for (uint8_t i = 0; i < BUTTON_COUNT; i++) {
    ...
    btn->raw_state = (pin_state == GPIO_PIN_RESET) ? TRUE : FALSE;

    // debouncing: button must be stable for BUTTON_DEBOUNCE_TIME_MS
    // increment counter if raw state matches current state, else reset
    if (btn->raw_state == btn->current_state) {
      // stable - reset debounce counter
      btn->debounce_counter = 0;
    } else {
      // state changed - increment debounce counter
      btn->debounce_counter++;

      // if stable long enough, update current state
      if (btn->debounce_counter >= BUTTON_DEBOUNCE_TIME_MS) {
